...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { countryCode, slug } = await params; const item = await getContentItem("pages", slug); if (!item) return {}
  return { title: item.seo?.seo_title || item.title, description: item.seo?.seo_description || item.excerpt, alternates: canonicalMetadata(countryCode, `pages/${slug}`), openGraph: { title: item.seo?.og_title || item.title, description: item.seo?.og_description || item.excerpt, images: item.seo?.og_image ? [item.seo.og_image] : undefined } }
}
