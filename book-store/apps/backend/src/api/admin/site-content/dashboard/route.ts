...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  const [[, pages], [, brands], [, articles], [, advertisements], [, drafts], recent] = await Promise.all([
    service.listAndCountContentPages({}, { take: 1 }),
    service.listAndCountBrands({}, { take: 1 }),
    service.listAndCountArticles({}, { take: 1 }),
    service.listAndCountAdvertisements({}, { take: 1 }),
  ...
  res.json({ counts: { pages, brands, articles, advertisements, drafts }, recent })
}
